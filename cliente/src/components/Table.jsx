  ...
  <thead className="bg-gray-50">
    <tr>
      {columns.map((title, index) => (
        <th
          scope="col"
          className={`px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${index === 2 ? 'hidden md:block' : ''}`}
          key={title}
        >
          {title}
        </th>
    ...
    {currentRows.map((row, rowIndex) => (
      <tr key={row.id || rowIndex} className="hover:bg-gray-100">
        {Object.values(row).map((cell, cellIndex) => (
          cell.length > 0 &&
          <td className={`px-6 py-4 whitespace-nowrap md:text-sm text-xs font-medium text-gray-900 ${cellIndex === 3 ? 'hidden md:block' : ''}`} key={cellIndex}>
            {cell}
          </td>
        ))}
       {renderCell && 
